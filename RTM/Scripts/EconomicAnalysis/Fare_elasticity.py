...
class DataExport(_m.Tool()):
    ...
    def export_transit_data(self, BAU_Path, Scen_Path):
        ...
        df.to_sql(name='fare_elasticity_table', con=conn, flavor='sqlite', index=False, if_exists='replace')
        df_rev.to_sql(name='fare_revenue_table', con=conn, flavor='sqlite', index=False, if_exists='replace')
        conn.close()
