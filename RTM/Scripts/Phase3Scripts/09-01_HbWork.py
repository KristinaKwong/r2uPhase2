...
#        ##############################################################################
        ...
        Df['BusTot'] = util.get_matrix_numpy(eb, 'HbWBlBAuBusIvtt') + util.get_matrix_numpy(eb, 'HbWBlBAuBusWait') + util.get_matrix_numpy(eb, 'HbWBlBAuBusAux')
...
# Store park and ride Demands separately for time slicing
      ...
      ## SOV Trips      #SOV*PA_Factor + SOV_transpose*AP_Factor
       ...
       # AM
        ...
        df_gy_phr.to_sql(name='phr_gy', con=conn, index=False, if_exists='replace')

        df_Daily_Gy.to_sql(name='daily_gy', con=conn, index=False, if_exists='replace')
