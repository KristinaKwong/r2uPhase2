...
def generate_od_df(eb, increment_dict, mode_dict, peak_facs, ensem='ga',ensem_agg=True, pa_flag = False):
    ...
    for purpose, increment in increment_dict.iteritems():
        for mode, mats in mode_dict.iteritems():
            ...
            df[label] = pa_to_od(eb, mat, peaks, pa_flag).flatten()
            ...
            if not ensem_agg:
                ...
                if counter == 1:
                    df.to_sql(name='od_daily', con=conn, index=False, if_exists='replace')
                else:
                    df.to_sql(name='od_daily', con=conn, index=False, if_exists='append')
            ...
            conn = util.get_db_byname(eb, "trip_summaries.db")
            if counter == 1:
                df.to_sql(name='od_daily_{}'.format(ensem), con=conn, index=False, if_exists='replace')
            else:
...
def main(eb, ensem='ga',ensem_agg=True, pa_flag = False):
    ...
    generate_od_df(eb, increment_dict, mode_dict, peak_facs, ensem, ensem_agg, pa_flag)
