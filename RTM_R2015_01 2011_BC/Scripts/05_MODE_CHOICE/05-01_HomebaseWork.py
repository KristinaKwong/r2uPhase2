...
def old_calculate_probabilities(scenario):
    ...
    ## Probabilities are saved in matrices mf441-mf503 and are calculated
    ## from utilities stored in matrices mf374-mf436
    ## Matrices mf924  -  mf927 are used to store intermediates

    tiny = str(0.000001)
    mfresult = 441
    startmf = 374
    theta = str(0.713222879234)


    spec_list = []
    for i in range(9):
        # Pre-calculate each "nest" probability and the denominator for the full nest

        auto_nest_util_expr = (
            "(exp(mf" + str(startmf + i + 0 * 9) + ") + exp(mf" + str(startmf + i + 1 * 9) + ")"
            + " + exp(mf" + str(startmf + i + 2 * 9) + ") + " + tiny + ")" + "^" + "(" + theta + ")")
        auto_nest_util = "mf925"
        spec_list.append(build_spec(auto_nest_util_expr, auto_nest_util))

        transit_nest_util_expr = (
            "(exp(mf" + str(startmf + i + 3 * 9) + ")" +
            " + exp(mf" + str(startmf + i + 4 * 9) + ") + " + tiny + ")" + "^" + "(" + theta + ")")
        transit_nest_util = "mf926"
        spec_list.append(build_spec(transit_nest_util_expr, transit_nest_util))

        active_nest_util_expr = (
            "(exp(mf" + str(startmf + i + 5 * 9) + ")" +
            " + exp(mf" + str(startmf + i + 6 * 9) + ")+" + tiny + ")" + "^" + "(" + theta + ")")
        active_nest_util = "mf927"
        spec_list.append(build_spec(active_nest_util_expr, active_nest_util))

        denominator_expr = "((" + auto_nest_util + ")+" + "(" + transit_nest_util + ")+" + "(" + active_nest_util + "))"
        denominator = "mf924"
        spec_list.append(build_spec(denominator_expr, denominator))
        ...
        for k in range(0, 3):
            newexp = (
                "((" + auto_nest_util + ")" + "/" + denominator + ")" + "*" + "(exp(mf" + str(startmf + i + k * 9) + "))" + "/"
                + "(" + tiny + " + exp(mf" + str(startmf + i + 0 * 9) + ") + exp(mf" + str(startmf + i + 1 * 9) + ")"
                + "+ exp(mf" + str(startmf + i + 2 * 9) + "))"
            )
            spec_list.append(build_spec(newexp, "mf" + str(mfresult + i + k * 9)))

        for k in range(3, 5):
            newexp = (
                "((" + transit_nest_util + ")" + "/" + denominator + ")" + "*" + "(exp(mf" + str(startmf + i + k * 9) + "))" + "/"
                + "(" + tiny + "+exp(mf" + str(startmf + i + 3 * 9) + ")+exp(mf" + str(startmf + i + 4 * 9) + "))"
            )
            spec_list.append(build_spec(newexp, "mf" + str(mfresult + i + k * 9)))
        for k in range(5, 7):
            newexp = (
                "((" + active_nest_util + ")" + "/" + denominator + ")" + "*" + "(exp(mf" + str(startmf + i + k * 9) + "))" + "/"
                + "(" + tiny + "+exp(mf" + str(startmf + i + 5 * 9) + ")+exp(mf" + str(startmf + i + 6 * 9) + "))"
            )
            spec_list.append(build_spec(newexp, "mf" + str(mfresult + i + k * 9)))
