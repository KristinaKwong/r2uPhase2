...
class ModeChoiceHBEsc(_modeller.Tool()):
    def page(self):
...
@_modeller.logbook_trace("Home-base Escorting")
def run_model(scenario, data_folder, iteration_number, is_last_iteration):
    ...
    time_slice_escorting(scenario, data_folder)
    calculate_final_period_demand(scenario)
    if is_last_iteration:
        utilities.export_matrices_report(data_folder, "esco", range(773, 843))


@_modeller.logbook_trace("continue aggregating non work demand, escorting")
...
@_modeller.logbook_trace("Calculate_Bus_utlity")
...
@_modeller.logbook_trace("Calculate_Home-base-escorting Blend Skims")
def calculate_blends(scenario):
    print "--------Calculate_Home-base_Escorting_blends, " + str(datetime.now().strftime('%H:%M:%S'))
    emmebank = scenario.emmebank
...
#********
def time_slice_escorting(scenario, data_folder):
    ...
    print "Time slicing Escorting matrices completed." + str(datetime.now().strftime('%H:%M:%S'))
...
def calculate_final_period_demand(scenario):
    ...
    spec_list.append(build_spec("mf851+" + "(mf784/" + msAutOccEcor2Plus + ")", "mf851"))
    spec_list.append(build_spec("mf852+" + "(mf805/" + msAutOccEcor2Plus + ")", "mf852"))
    ...
    spec_list.append(build_spec("mf864+" + "(mf785/" + msAutOccEcor2PlusM + ")", "mf864"))
    spec_list.append(build_spec("mf865+" + "(mf806/" + msAutOccEcor2PlusM + ")", "mf865"))
    ...
    spec_list.append(build_spec("mf877+" + "(mf786/" + msAutOccEcor2Plus + ")", "mf877"))
    spec_list.append(build_spec("mf878+" + "(mf807/" + msAutOccEcor2Plus + ")", "mf878"))
