...
class ModeChoiceHBShopping(_m.Tool()):
    ...
    def __call__(self, eb, scenario, is_last_iteration):
        ...
        self.time_slice_shopping(eb, scenario)
...
#            utilities.export_matrices_report(eb, "shop", range(773, 843))
    ...
    def calculate_bike(self, scenario):
        ...
        expression_2 = (bkscr_bk +
                        "*((mo13+md13).gt.5)*(gy(p).ne.4)*(gy(p).ne.3)*(gy(q).ne.4)*(gy(q).ne.3)*(gy(p).lt.12)*(gy(q).lt.12)")
        ...
        expression_3 = bk_van + "*(((gy(p).eq.4)+(gy(q).eq.4)).ge.1)"
    ...
    def calculate_walk(self, scenario):
        ...
        for i in range(1, 10):
            expression_1 = alt_spec_cons
            expression_1 = expression_1 + " -0.3*(((gy(p).eq.6)+(gy(q).eq.6)).ge.1)"
            if i < 4:
                expression_1 = expression_1 + " + " + low_inc + "*((gy(p).ne.3)*(gy(q).ne.3)*(gy(p).lt.12)*(gy(q).lt.12))"
            if i in (1, 4, 7):
                expression_1 = expression_1 + " + " + zero_cars
            if i in (3, 6, 9):
                #1 (if cars = 2/3) * ifne(gyo,5)*ifne(gyd,5)* ifne(gyo,3)*ifne(gyd,3)* ifne(gyo,4)*ifne(gyd,4)
                expression_1 = expression_1 + " + " + two_plus_car + \
                               "*((gy(p).ne.3)*(gy(p).ne.4)*(gy(p).ne.5)*(gy(q).ne.3)*(gy(q).ne.4)*(gy(q).ne.5))"
    ...
    def calculate_rail(self, scenario):
        ...
        for i in range(1, 10):
            ...
            expression_1 = (expression_1 + " -0.2*(((gy(p).eq.4).or.(gy(q).eq.4)))"
                                           " -0.15*(((gy(p).eq.7).or.(gy(q).eq.7)))"
                                           " -0.15*(((gy(p).eq.8).or.(gy(q).eq.8)))")
            if i > 6:
                expression_1 = expression_1 + " + " + high_inc
            if i < 7:
                expression_1 = (expression_1 + " + " + nhi_intrv +
                                "*((gy(p).eq.4)*(gy(q).eq.4))+0.5*(gy(p).eq.3)-0.3*(gy(p).eq.4)")
            if i in (1, 4, 7):
                expression_1 = expression_1 + " + " + zero_cars
    ...
    def calculate_bus(self, scenario):
        ...
        for i in range(1, 10):
            ...
            expression_1 = (expression_1 + " -0.15*(((gy(p).eq.8).or.(gy(q).eq.8))) "
                                           " - 0.2*(((gy(p).eq.10).or.(gy(q).eq.10)))"
                                           " + 0.2*(((gy(p).eq.13).or.(gy(q).eq.13)))")
            expression_1 = (expression_1 + " - 0.15*(((gy(p).eq.2).or.(gy(q).eq.2))) "
                                           " - 0.15*(((gy(p).eq.4).or.(gy(q).eq.4)))")
            if i < 7:
                expression_1 = expression_1 + "-0.4*(gy(p).eq.2)-0.2*(gy(p).eq.6)-0.3*(gy(p).eq.4)-0.2*(gy(p).eq.7)"
            if i > 6:
                expression_1 = expression_1 + " + " + high_inc
            if i in (1, 4, 7):
                expression_1 = expression_1 + " + " + zero_cars
    ...
    def calculate_blends(self, scenario):
        ...
        specs = []
        specs.append(util.matrix_spec("mf140", "(mf110.eq.1)*(ms53+((mf115.eq.0)*(1-ms53)))"))
        specs.append(util.matrix_spec("mf141", "1-mf140"))
        specs.append(util.matrix_spec("mf142", "(mf121.eq.1)*(ms53+(((mf129.eq.0)+(mf130.eq.0)).ge.1)*(1-ms53))"))
        specs.append(util.matrix_spec("mf143", "1-mf142"))
        specs.append(util.matrix_spec("mf144", "mf100*ms53+mf103*(1-ms53)"))
        specs.append(util.matrix_spec("mf145", "mf101*ms53+mf104*(1-ms53)"))
        specs.append(util.matrix_spec("mf146", "mf102*ms53+mf105*(1-ms53)"))
    ...
    def calculate_final_period_demand(self, scenario):
        ...
        specs.append(util.matrix_spec("mf846", "mf846+" + "mf777*.35"))
        specs.append(util.matrix_spec("mf847", "mf847+" + "mf798*.35"))
        specs.append(util.matrix_spec("mf851", "mf851+" + "(mf784*.35/" + msAutOccShop2Plus + ")"))
        specs.append(util.matrix_spec("mf852", "mf852+" + "(mf805*.35/" + msAutOccShop2Plus + ")"))
        ...
        specs.append(util.matrix_spec("mf864", "mf864+" + "(mf785/" + msAutOccShop2PlusM + ")"))
        specs.append(util.matrix_spec("mf865", "mf865+" + "(mf806/" + msAutOccShop2PlusM + ")"))
        ...
        specs.append(util.matrix_spec("mf877", "mf877+" + "(mf786/" + msAutOccShop2Plus + ")"))
        specs.append(util.matrix_spec("mf878", "mf878+" + "(mf807/" + msAutOccShop2Plus + ")"))
